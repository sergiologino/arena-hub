...
import { ClipboardCheck, Search, Shield, Smartphone, Scale, Zap, Globe, Loader2, Check, ArrowRight, ChevronDown, Eye, ArrowLeft } from "lucide-react"
...
export default function ShipPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setLoading(true)
    try {
      const res = await fetch("/api/check", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ url: cleanUrl, promoCode }) })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error || "Check failed")
      router.push(`/ship/report/${data.id}`)
    } catch (err: any) {
    ...
    <div className="min-h-screen">
      <nav className="fixed top-0 w-full z-50 glass border-b border-white/5">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            <Link href="/history" className="text-sm text-dark-300 hover:text-white transition">My Audits</Link>
        ...
        <div className="relative max-w-4xl mx-auto text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <h1 className="text-5xl sm:text-7xl font-black mb-6 leading-tight">
              Never launch<br /><span className="bg-clip-text text-transparent bg-gradient-to-r from-emerald-400 to-cyan-400">unprepared</span> again
            </h1>
            ...
            <div className="mt-4">
              {!showPromo ? (
                <button onClick={() => setShowPromo(true)} className="text-sm text-dark-400 hover:text-emerald-400 transition">Have a promo code?</button>
              ) : (
              ...
              <p className="text-sm text-dark-400 mt-2">Free quick audit - Full report $7.99</p>
            </div>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-20 px-4">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-3xl sm:text-4xl font-bold text-center mb-4">What we <span className="bg-clip-text text-transparent bg-gradient-to-r from-emerald-400 to-cyan-400">check</span></h2>
      ...
      <section id="pricing" className="py-20 px-4 bg-dark-900/50">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-3xl font-bold text-center mb-12">Simple <span className="bg-clip-text text-transparent bg-gradient-to-r from-emerald-400 to-cyan-400">pricing</span></h2>
      ...
      <section id="faq" className="py-20 px-4">
        <div className="max-w-2xl mx-auto">
          <h2 className="text-3xl font-bold text-center mb-12"><span className="bg-clip-text text-transparent bg-gradient-to-r from-emerald-400 to-cyan-400">FAQ</span></h2>
      ...
      <footer className="py-8 px-4 border-t border-white/5">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4 text-sm text-dark-400">
            ...
            <Link href="/paywall" className="hover:text-purple-400 transition">Paywall Audit</Link>
          </div>
        </div>
      </footer>
    </div>
