...
import CheckCategoryCard from "@/components/ship/CheckCategoryCard"
...
function saveToHistory(data: CheckReport) {
  try {
    const key = "shipchecklist_history"
    const stored = localStorage.getItem(key)
    const history = stored ? JSON.parse(stored) : []
    const item = { id: data.id, url: data.url, score: data.overallScore, createdAt: data.createdAt, tool: "ship" }
...
export default function ReportClient({ id }: { id: string }) {
  ...
  const fetchReport = useCallback(async () => {
    try {
      const res = await fetch(`/api/report/${id}?tool=ship`)
  ...
  if (loading) return <LoadingAnimation tool="ship" />
  if (error || !report) return (
    <div className="min-h-screen flex items-center justify-center">
      <div className="text-center">
        <AlertTriangle className="w-12 h-12 text-red-500 mx-auto mb-4" />
        <p className="text-red-400 mb-4">{error || "Report not found"}</p>
        <Link href="/ship" className="text-emerald-500 hover:underline">Back to home</Link>
      </div>
    </div>
  )
    ...
    <div className="min-h-screen pb-20">
      <nav className="sticky top-0 z-40 glass border-b border-white/5">
        <div className="max-w-5xl mx-auto px-4 py-4 flex items-center justify-between">
          <Link href="/ship" className="flex items-center gap-2 text-dark-300 hover:text-white transition">
      ...
      <div className="max-w-4xl mx-auto px-4 pt-8">
        ...
        <div className="mt-12 mb-8"><ShareButtons score={report.overallScore} url={report.url} reportId={id} toolPath="ship" toolName="ShipChecklist.ai" /></div>
        <div className="text-center"><Link href="/ship" className="text-emerald-500 hover:underline text-sm">Check another site</Link></div>
      </div>

      {showPayment && <PaymentModal reportId={id} walletAddress={WALLET} price={PRICE} apiPath="/api/payment/verify" gradientClass="from-emerald-500 to-cyan-500" accentColor="emerald-500" onSuccess={() => { setShowPayment(false); fetchReport() }} onClose={() => setShowPayment(false)} />}
    </div>
  )
}
