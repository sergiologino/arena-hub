...
import { roastStore, shipStore, paywallStore, findReportAnyTool } from "@/lib/store"
...
export async function POST(request: NextRequest) {
  try {
    ...
    const found = await findReportAnyTool(reportId)
    if (!found) return NextResponse.json({ error: "Report not found" }, { status: 404 })
    if (found.report.isPaid) return NextResponse.json({ status: "confirmed", message: "Already paid" })
