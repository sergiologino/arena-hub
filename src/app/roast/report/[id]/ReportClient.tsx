...
import CategoryCard from "@/components/roast/CategoryCard"
...
function saveToHistory(report: RoastReport) {
  try {
    ...
    const item = { id: report.id, url: report.url, score: report.overallScore, createdAt: report.createdAt, tool: "roast" }
...
export default function ReportClient({ id }: { id: string }) {
  ...
  const fetchReport = useCallback(async () => {
    try {
      const res = await fetch(`/api/report/${id}?tool=roast`)
  ...
  if (loading) return <LoadingAnimation tool="roast" />
  if (error || !report) return (
    <div className="min-h-screen flex items-center justify-center">
      <div className="text-center">
        <AlertTriangle className="w-12 h-12 text-red-500 mx-auto mb-4" />
        <p className="text-red-400 mb-4">{error || "Report not found"}</p>
        <Link href="/roast" className="text-orange-500 hover:underline">Back to home</Link>
      </div>
    </div>
  )
    ...
    <div className="min-h-screen pb-20">
      <nav className="sticky top-0 z-40 glass border-b border-white/5">
        <div className="max-w-5xl mx-auto px-4 py-4 flex items-center justify-between">
          <Link href="/roast" className="flex items-center gap-2 text-dark-300 hover:text-white transition">
      ...
      <div className="max-w-4xl mx-auto px-4 pt-8">
        ...
        <div className="mt-12 mb-8">
          <ShareButtons score={report.overallScore} url={report.url} reportId={id} toolPath="roast" toolName="RoastPage.ai" />
        </div>
        <div className="text-center"><Link href="/roast" className="text-orange-500 hover:underline text-sm">Roast another page</Link></div>
      </div>

      {showPayment && <PaymentModal reportId={id} walletAddress={WALLET} price={PRICE} apiPath="/api/payment/verify" gradientClass="from-orange-500 to-red-500" accentColor="orange-500" onSuccess={() => { setShowPayment(false); fetchReport() }} onClose={() => setShowPayment(false)} />}
    </div>
  )
}
