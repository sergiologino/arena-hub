...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  try {
    ...
    return {
      title, description,
      openGraph: { title, description, type: "website", url: `${base}/roast/share/${params.id}`, siteName: "App Audit", images: [{ url: ogImage, width: 1200, height: 630, alt: title }] },
...
export default function SharePage({ params }: Props) {
  return <ShareRedirect id={params.id} toolPath="roast" />
}
