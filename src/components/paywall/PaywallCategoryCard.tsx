...
import { PaywallCategory } from "@/lib/types"
...
export default function PaywallCategoryCard({ category, index, locked }: { category: PaywallCategory; index: number; locked?: boolean }) {
  if (locked) {
    return (
      ...
      <div className="mb-4">
        ...
        <div className="h-2 bg-dark-800 rounded-full overflow-hidden">
          <motion.div initial={{ width: 0 }} animate={{ width: `${category.score}%` }} transition={{ duration: 1, delay: index * 0.1 + 0.3 }}
            className={`h-full rounded-full ${category.score >= 80 ? "bg-green-500" : category.score >= 60 ? "bg-yellow-500" : category.score >= 40 ? "bg-purple-500" : "bg-red-500"}`} />
        </div>
      </div>
          ...
          <ul className="space-y-2">
            {category.recommendations.map((rec, j) => (
              <li key={j} className="flex items-start gap-2 text-sm text-dark-200">
                <span className="text-purple-500 mt-0.5 flex-shrink-0">{'\u2192'}</span><span>{rec}</span>
              </li>
            ))}
          </ul>
        </div>
