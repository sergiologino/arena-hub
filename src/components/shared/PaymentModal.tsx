...
export default function PaymentModal({ reportId, walletAddress, price, apiPath, accentColor = "orange-500", gradientClass = "from-orange-500 to-red-500", onSuccess, onClose }: Props) {
  ...
  async function verify() {
    ...
    try {
      const res = await fetch(apiPath, {
        ...
        body: JSON.stringify({ reportId, txHash: txHash.trim() }),
      })
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="glass-card max-w-md w-full relative">
        ...
        <div className="text-center mb-6">
          <Wallet className={`w-10 h-10 text-${accentColor} mx-auto mb-3`} />
          <h3 className="text-xl font-bold">Unlock Full Report</h3>
          <div className={`text-3xl font-black text-${accentColor} mt-2`}>${price}</div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input type="text" value={txHash} onChange={e => setTxHash(e.target.value)} placeholder="0x..."
              className={`w-full px-4 py-3 bg-dark-800 border border-dark-600 rounded-xl text-white placeholder:text-dark-500 focus:outline-none focus:ring-2 focus:ring-${accentColor} text-sm font-mono`} />
          </div>
          <button onClick={verify} disabled={verifying || !txHash.trim()}
            className={`w-full py-3.5 bg-gradient-to-r ${gradientClass} rounded-xl text-white font-bold transition disabled:opacity-50 flex items-center justify-center gap-2`}>
